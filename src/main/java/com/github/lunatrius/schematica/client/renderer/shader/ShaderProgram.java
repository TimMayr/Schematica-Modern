...
public class ShaderProgram {
	...
	private int loadAndCompileShader(String domain, String filename, int shaderType) {
		...
		if (handle == 0) {
			Reference.logger.error("Could not create shader of type {} for {}: {}", shaderType, filename,
		...
		if (GL20.glGetShaderi(handle, GL20.GL_COMPILE_STATUS) == GL11.GL_FALSE) {
			Reference.logger.error("Could not compile shader {}: {}", filename,
			                       GL20.glGetShaderInfoLog(this.program, 1024));
			GL20.glDeleteShader(handle);
			return 0;
		}
