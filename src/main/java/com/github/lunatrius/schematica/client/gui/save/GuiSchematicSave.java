...
public class GuiSchematicSave extends ScreenBase {
	...
	public void initGui() {
		...
		this.btnPointA =
				new Button(id++, this.centerX - 130, this.centerY - 55, 100, 20,
		...
		this.btnPointB =
				new Button(id++, this.centerX + 30, this.centerY - 55, 100, 20,
		...
		this.btnEnable = new Button(id++, this.width - 210, this.height - 55, 50, 20,
		...
		this.btnSave.enabled =
				ClientProxy.isRenderingGuide && Schematica.proxy.isSaveEnabled || ClientProxy.schematic != null;
		this.buttonList.add(this.btnSave);

		this.btnFormat = new Button(id++, this.width - 155, this.height - 55, 145, 20,
		                            I18n.format(Names.Gui.Save.FORMAT,
		                                        I18n.format(SchematicFormat.getFormatName(this.format))));
		this.btnFormat.enabled =
				ClientProxy.isRenderingGuide && Schematica.proxy.isSaveEnabled || ClientProxy.schematic != null;
	...
	protected void actionPerformed(Button Button) {
		if (Button.enabled) {
			if (Button.id == this.btnPointA.id) {
				...
				this.format = nextFormat();
				this.btnFormat.displayString =
						I18n.format(Names.Gui.Save.FORMAT, I18n.format(SchematicFormat.getFormatName(this.format)));
				...
				if (ClientProxy.isRenderingGuide) {
					if (Schematica.proxy.saveSchematic(this.mc.player, SchematicaClientConfig.schematicDirectoryPath,
					                                   path, this.mc.world, this.format, ClientProxy.pointMin,
					                                   ClientProxy.pointMax)) {
						this.filename = "";
						this.tfFilename.setText(this.filename);
					}
				} else {
					SchematicFormat.writeToFileAndNotify(new File(SchematicaClientConfig.schematicDirectoryPath, path),
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawString(this.fontRenderer, Integer.toString(ClientProxy.pointA.x), this.centerX - 25, this.centerY - 24,
		           0xFFFFFF);

		drawString(this.fontRenderer, this.strY, this.centerX - 145, this.centerY + 1, 0xFFFFFF);
		drawString(this.fontRenderer, Integer.toString(ClientProxy.pointA.y), this.centerX - 25, this.centerY + 1,
		           0xFFFFFF);

		drawString(this.fontRenderer, this.strZ, this.centerX - 145, this.centerY + 26, 0xFFFFFF);
		drawString(this.fontRenderer, Integer.toString(ClientProxy.pointA.z), this.centerX - 25, this.centerY + 26,
		           0xFFFFFF);

		drawString(this.fontRenderer, this.strX, this.centerX + 15, this.centerY - 24, 0xFFFFFF);
		drawString(this.fontRenderer, Integer.toString(ClientProxy.pointB.x), this.centerX + 135, this.centerY - 24,
		           0xFFFFFF);

		drawString(this.fontRenderer, this.strY, this.centerX + 15, this.centerY + 1, 0xFFFFFF);
		drawString(this.fontRenderer, Integer.toString(ClientProxy.pointB.y), this.centerX + 135, this.centerY + 1,
		           0xFFFFFF);

		drawString(this.fontRenderer, this.strZ, this.centerX + 15, this.centerY + 26, 0xFFFFFF);
		drawString(this.fontRenderer, Integer.toString(ClientProxy.pointB.z), this.centerX + 135, this.centerY + 26,
		           0xFFFFFF);
