...
public interface PreSchematicSaveCallback {
	Event<PreSchematicSaveCallback> EVENT =
			EventFactory.createArrayBacked(PreSchematicSaveCallback.class,
			                               (listeners) -> (ISchematic schematic, Map<String, Block> mappings,
			                                               CompoundTag extendedMetadata) -> {
				                               if (extendedMetadata == null) {
					                               extendedMetadata = new CompoundTag();
				                               }

				                               for (PreSchematicSaveCallback listener : listeners) {

					                               boolean result =
							                               listener.preSave(schematic, mappings, extendedMetadata);
